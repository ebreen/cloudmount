...
must_haves:
  truths:
    ...
    - "openItem serves from local cache when a staging file already exists"
...
- B2VolumeReadWrite.swift: open, close, read, write
  ...
  <verify>
1. Build succeeds: `xcodebuild -scheme CloudMount clean build CODE_SIGNING_REQUIRED=NO CODE_SIGNING_ALLOWED=NO 2>&1 | grep -E "BUILD (SUCCEEDED|FAILED)"` outputs `BUILD SUCCEEDED`.
2. All 9 extension Swift files exist: `ls CloudMountExtension/*.swift | wc -l` outputs 9.
3. All FSVolume protocol requirements satisfied (no missing methods in build output).

If compilation errors occur, fix them — common FSKit API issues to check:
- `FSFileName` constructor: might be `FSFileName(string:)` or `FSFileName(_ string:)` — match actual API
- `FSDirectoryCookie`: might be a struct with specific initializer or typealias for UInt64
...
- `FSItem.Attributes` setters: verify property names (`type`, `mode`, `uid`, `gid`, `size`, `linkCount`, `modifyTime`, `birthTime`)
  </verify>
  <done>File I/O operations complete: openItem downloads from B2, read serves from local cache, write goes to staging, closeItem uploads dirty files to B2. Upload failures preserve staged files. Full project clean build succeeds with all 9 extension Swift files. All FSVolume protocol methods implemented. Extension is structurally complete and ready for runtime mount testing in Phase 7.</done>
</task>
...
- All FSVolume protocol requirements satisfied (no missing methods)

Note: Runtime mount-and-browse verification (FSKIT-07) is deferred to Phase 7 (App Integration), which wires the host app to mount the extension and provides the end-to-end Finder testing context.
</verification>

<success_criteria>
FSKit filesystem extension is structurally complete: all volume operations (browse, read, write, delete, rename) are implemented and wired to B2Client. The project builds cleanly with zero errors. Extension is ready for runtime mount testing in Phase 7 (App Integration).
</success_criteria>
