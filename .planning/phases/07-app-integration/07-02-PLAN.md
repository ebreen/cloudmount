...
must_haves:
  ...
  key_links:
    ...
    - from: "CloudMount/AppState.swift"
      ...
      via: "mountMonitor property + Combine sink on $mountedPaths to sync external mount/unmount changes into mountStatuses"
      pattern: "mountMonitor\\.\\$mountedPaths"
    - from: "CloudMount/AppState.swift"
...
Import `Combine` at the top of the file.
...
- `private var mountPathCancellable: AnyCancellable?` — stores the Combine subscription for external mount changes
...
func startMonitoring() {
    ...
    // Subscribe to MountMonitor's mountedPaths to detect external mount/unmount
    // (e.g. Finder eject, diskutil from Terminal, other tools).
    // When mountedPaths changes, reconcile mountStatuses for all configs.
        ...
        .sink { [weak self] mountedPaths in
            ...
            for config in self.mountConfigs {
                ...
                if isCurrentlyMounted && currentStatus != .mounted {
                    ...
                    // Was mounted, now gone — external unmount
...
Update `clearAll()` to also call `mountMonitor.stopMonitoring()`, clear `mountStatuses`, and cancel the Combine subscription: `mountPathCancellable?.cancel(); mountPathCancellable = nil`.
...
**CloudMountApp.swift — Add monitoring lifecycle:**

Add `@State private var hasStartedMonitoring = false` to the App struct.

Add `.onAppear` with a guard to the MenuContentView inside MenuBarExtra. `.onAppear` on MenuBarExtra content fires each time the popover opens, so the guard ensures `startMonitoring()` runs only once:
...
No other changes to CloudMountApp.swift. Onboarding sheet is presented from MenuContentView (below).

**MenuContentView.swift — Add onboarding sheet (additional to Task 1):**

Add `.sheet(isPresented: $appState.showOnboarding)` modifier on the outermost VStack in MenuContentView's body:
...
MenuBarExtra with `.window` style supports `.sheet` presentation.
  </action>
